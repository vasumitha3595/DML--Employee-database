...
The supplied SQL contains schema changes that rename some tables and columns, while later statements still use the earlier names. For example, `departments` is renamed to `Departments_Info`, `Location` is renamed to `Locations`, and `hire_date` is renamed to `date_of_joining`.

Before running the entire script as one file, keep the table and column names consistent throughout the script.
